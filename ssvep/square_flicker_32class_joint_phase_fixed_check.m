function square_flicker_32class_joint_phase_fixed_check(preload)
%%Initiate frequency
...
phase_list =[] ;
phase_set=[0 pi*0.5 pi 1.5*pi 0 pi*0.5 pi 1.5*pi 0 pi*0.5 pi 1.5*pi 0 pi*0.5 pi 1.5*pi];
for i = 1 : 5
...
try
    ...
    Screen('Preference', 'OverrideMultimediaEngine',0);
    Screen('Preference', 'FrameRectCorrection', 1);
    Screen('Preference', 'TextRenderer', 1);
    Screen('Preference','TextEncodingLocale','UTF-8');
    
    %     VBLSyncTest(10000, 0, 0.6, 0, 0, 1, 0,0,0,2) ;
    ...
    A = imread('speller parameter.PNG');
    B = imresize(A,[1080 1920]);
    C = Screen('MakeTexture', win,uint8(B));
    Screen('DrawTexture', win,C);
    Screen('Flip', win);
    if (isempty(preload))
        flicker_images = [];
...
%         save('pre_texture.mat', '-v7.3', 'flicker_images');
    ...
    for trial_2 = 1: ceil(class/stimul_size)
        ...
        for trial_1  =1: length(stimul_set)
            ...
            imageArray = Screen('GetImage', win, [0 0 1920 160]);
            ...
            Openvibe_tcp(t, 20+stimul_set(trial_1)); %%  stm based epoch
...
%             tic;
            for indexflip = 1: len_Freq  % 1 trial
                ...
                stp = Screen('Flip', win);
                pulseAmp(trial_2,trial_1,:,indexflip) = 1-freqCombine(:, indexflip);
                timestamp(trial_2,trial_1,indexflip) = stp;
            end
%             toc;
            Openvibe_tcp(t, 3);
            tic;
