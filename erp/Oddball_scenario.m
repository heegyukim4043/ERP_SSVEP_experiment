...
num_target = round(num_trial*class_ratio + r);
disp(num_target);

image_type = 'oddball'; %'kr'
...
class_list  = ones(1,num_trial) ;
class_list(1:num_target) = 2;
...
%     [win,winRect] =   Screen(myScreen, 'OpenWindow');  % window size
    ...
    clear img
    [img] = imread(sprintf('images/%s/green.tif',image_type));
    texture_set{3} = Screen('MakeTexture',win,img); 
    clear img
    [img] = imread(sprintf('images/%s/red.tif',image_type));
